...
namespace AvatarPluginForUnity
{
    ...
    internal class AvatarMaterialCombiner
    {
        ...
        /// <returns></returns>
        public static (Dictionary<Material, TextureInfoBase>, Material) CombineMaterial(List<Material> materialSources, MaterialCombineOption materialCombineOption)
        {
            var (textureInfoBaseSet,
                materialTextureInfoDic) = MakeTextureInfoBases(materialSources, materialCombineOption.textureResolutionRatio);
            ...
            return (materialTextureInfoDic, MakeCombinedMaterial(combinedmainTexture,
                combinedNormalTexture,
                combinedAo_metallicRoughnessTexture, materialCombineOption.customMaterialData));
        }
        /// <summary>
        ...
        private static Material MakeCombinedMaterial(Texture2D combinedmainTexture,
            ...
            Texture2D combinedAo_metallicRoughnessTexture, CustomMaterialData customMaterialData)
        {
            var combinedMat = new Material(Shader.Find("glTF/PbrMetallicRoughness"));
...
/*#if UNITY_EDITOR
            combinedMat.SetFloat(Shader.PropertyToID("_Mode"), (int)StandardShaderMode.Cutout);
#endif*/
            ...
            return customMaterialData == null?combinedMat: customMaterialData.GenerateCustomMaterialFromGLTFastPBRMaterial(combinedMat);
        }
        ...
        /// <returns></returns>
        public static bool MaterialCombineVerification(Material material, CustomMaterialData customMaterialData, bool strictCheck = false)
        {
            ...
            if (customMaterialData != null)
                (mainTexture, normalTexture, Ao_metallicRoughnessTexture) = customMaterialData.GetTexturesFromCustomMaterial(material);
            else
            {
            ...
            if (mainTexture == null)
            {
                Debug.LogError("Materials that cannot be combined!!! There is no mainTexture!!");
                return false;
            }
            ...
            {
                Debug.LogError("Materials that cannot be combined!!! Neither width nor height, or either one, satisfies POT(Power Of Two).!!");
                return false;
            }
        ...
        /// <param name="material">The material.</param>
        /// <returns></returns>
        public static Material CopyNewMaterial(Material material, MeshCombineOption meshCombineOption)
        {


            MaterialCombineOption materialCombineOption = (meshCombineOption ==null|| !meshCombineOption.combineFlags.HasFlag(CombineFlags.UseMaterialCombine)) ? null : meshCombineOption.materialCombineOption;
            CustomMaterialData customMaterialData = materialCombineOption==null?null:materialCombineOption.customMaterialData;
            int resolutionRatio = materialCombineOption == null ? 1: (int)materialCombineOption.textureResolutionRatio;
            ...
            if (customMaterialData != null)
            {
                customMaterialData.SetTexturesToCustomMaterial(newMaterial, mainTexture, normalTexture, Ao_metallicRoughnessTexture);
            }
            else
            {
                if (mainTexture != null)
